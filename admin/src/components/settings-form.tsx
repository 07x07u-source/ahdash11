...
export function SettingsForm({ items }: { items: GameSettingItem[] }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const settings = items.filter((item) => values[item.key] !== item.value).map((item) => ({ key: item.key, value: values[item.key] }));
      if (!settings.length) { setMessage("لم تتغير أي قيمة."); return; }
  ...
  return (
    <form onSubmit={submit} className="surface-card admin-settings-form overflow-hidden" aria-busy={busy}>
      ...
      {!items.length ? <p role="status" className="p-6 text-sm text-[var(--muted)]">لا توجد إعدادات رقمية متاحة للتعديل. تحقق من الاتصال والصلاحيات في صحة الأنظمة.</p> : null}
      <p className="px-5 pt-4 text-xs leading-6 text-[var(--muted)]">تظهر هنا القيم الرقمية فقط. قواعد Party المركبة والقيم المنطقية لا تُحوّل إلى أرقام عند الحفظ.</p>
      <div className="grid gap-3 p-5 md:grid-cols-2">
        {items.map((item) => (
          <label key={item.key} className="rounded-xl border border-white/7 bg-white/[0.018] p-3">
            ...
            <input type="number" min={0} max={1000000} step="any" disabled={busy} value={values[item.key]} onChange={(event) => setValues((current) => ({ ...current, [item.key]: Number(event.target.value) }))} className="field mt-3" dir="ltr" required />
          </label>
        ))}
      </div>
      ...
      <div className="flex justify-end border-t border-white/7 p-4"><button type="submit" disabled={busy || !items.length} className="button-primary">{busy ? <LoaderCircle size={16} className="animate-spin" /> : <Save size={16} />} حفظ التغييرات</button></div>
    </form>
  );
}
